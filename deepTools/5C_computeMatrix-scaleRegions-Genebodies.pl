...
my $output_g = '';  ## such as "5C_computeMatrix-scaleRegions-Genebodies"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Usage:
               perl  5C_computeMatrix-scaleRegions-Genebodies.pl    [-version]    [-help]      [-in inputDir]     [-bed bedDir]     [-out outDir]
        For instance:
               nohup perl  5C_computeMatrix-scaleRegions-Genebodies.pl    -in 1_bamCoverage   -bed GenomicRegions    -out 5C_computeMatrix-scaleRegions-Genebodies    > 5C_computeMatrix-scaleRegions-Genebodies.runLog.txt 2>&1   &
...
## Version Infromation
my $version = "version 1.1,  2023-11-05.";
...
$output_g   = '5C_computeMatrix-scaleRegions-Genebodies';   ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  5C_computeMatrix-scaleRegions-Genebodies.pl  -help' \n";
    exit 0;
}
...
###################################################################################################################################################################################################
say   "\n\n\n\n\n\n##################################################################################################";
say   "Using computeMatrix  scale-regions, plotProfile and plotHeatmap ......";

for ( my $i=0; $i<=$#BED_Files_g; $i++ ) { 
    next unless $BED_Files_g[$i] =~ m/\.gtf$/;
    ...
    for ( my $j=0; $j<=$#BigWigfiles_g; $j++ ) {
         ...
         system("computeMatrix    scale-regions    --scoreFileName $input_g/$BigWigfiles_g[$j]    --regionsFileName $BEDdir_g/$BED_Files_g[$i]     --smartLabels   --numberOfProcessors 16   --verbose    -o $output_sub1/1.gzipped-matrix.gz    --outFileNameMatrix $output_sub1/1.matrix.txt   --outFileSortedRegions  $output_sub1/1.Regions.txt    --regionBodyLength  4000   --upstream 2000   --downstream 2000  --binSize 10    --missingDataAsZero    --transcriptID  transcript  >> $output_sub1/1.runLog.txt   2>&1");                               
