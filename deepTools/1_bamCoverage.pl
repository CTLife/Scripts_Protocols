...
my $output_g = '';  ## such as "deepTools/1_bamCoverage"
...
my $HELP = '
        ...
        Usage:
               perl  1_bamCoverage.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup  perl  1_bamCoverage.pl    -in 5_noDups/2_Bowtie2   -out deepTools/1_bamCoverage    > 1_bamCoverage.runLog.txt  2>&1   &
...
$output_g = 'deepTools/1_bamCoverage';      ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  1_bamCoverage.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        system(" bamCoverage   --extendReads     --bam $input_g/$temp.bam     --outFileName $output_g/$temp.bw     --outFileFormat bigwig    --binSize 10    --numberOfProcessors 16      --verbose       --normalizeUsing RPKM   --ignoreForNormalization chrX chrY chrM            > $output_g/$temp.runLog.txt  2>&1  ");                                                  
