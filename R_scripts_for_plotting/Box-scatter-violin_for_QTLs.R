...
numSample <- args[2]   ## number of samples
outputDir <- args[3]   ## Output dir
## Rscript   Box-scatter-violin.R    Final.1_BA9.full_info.txt   21   1_BA9 


#  inputFile <- "Final.1_BA9.full_info.txt"
#  numSample <- 21
#  outputDir <- "1_BA9.full_info"
...
###########################################################################################################################################################################
DF_1 <- read.table(inputFile, header=TRUE,   sep="\t" , comment.char = "" )  
dim( DF_1 )
#  DF_1[1:10,] 

distance = DF_1$dist_phe_var 
## hist(distance, breaks = 2000)
## length( distance )
## length( distance[ abs(distance) < 500000  ] )


numSamples = as.numeric( numSample )
numSamples


DF_phe = DF_1[,24:(24+numSamples-1)] 
DF_gno = DF_1[,(24+numSamples+9):(24+numSamples+8+numSamples)] 
dim( DF_phe )
dim( DF_gno )
#  DF_phe[1:10,] 
#  DF_gno[1:10,]
...
for( i in c(1:nrow(DF_gno)) ) {
  ## i = 1
  temp1 =  as.numeric( table(DF_gno[i,]) )
  temp2 =  names( table(DF_gno[i,]) )
  DF_gno_times[i,1] = 0
  DF_gno_times[i,2] = 0
  DF_gno_times[i,3] = 0
  if( length(temp2) == 2 ){  temp2[3] = 0; temp2[4] = 0 }
  if( length(temp2) == 3 ){  temp2[4] = 0 }
  
  if( temp2[1] == "0|0" ) { DF_gno_times[i,1] = DF_gno_times[i,1] + temp1[1] }
  if( temp2[2] == "0|0" ) { DF_gno_times[i,1] = DF_gno_times[i,1] + temp1[2] }
  if( temp2[3] == "0|0" ) { DF_gno_times[i,1] = DF_gno_times[i,1] + temp1[3] }
  if( temp2[4] == "0|0" ) { DF_gno_times[i,1] = DF_gno_times[i,1] + temp1[4] }
  
  if( temp2[1] == "0|1" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[1] }
  if( temp2[2] == "0|1" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[2] }
  if( temp2[3] == "0|1" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[3] }
  if( temp2[4] == "0|1" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[4] }
  
  if( temp2[1] == "1|0" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[1] }
  if( temp2[2] == "1|0" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[2] }
  if( temp2[3] == "1|0" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[3] }
  if( temp2[4] == "1|0" ) { DF_gno_times[i,2] = DF_gno_times[i,2] + temp1[4] }
  
  if( temp2[1] == "1|1" ) { DF_gno_times[i,3] = DF_gno_times[i,3] + temp1[1] }
  if( temp2[2] == "1|1" ) { DF_gno_times[i,3] = DF_gno_times[i,3] + temp1[2] }
  if( temp2[3] == "1|1" ) { DF_gno_times[i,3] = DF_gno_times[i,3] + temp1[3] }
  if( temp2[4] == "1|1" ) { DF_gno_times[i,3] = DF_gno_times[i,3] + temp1[4] }
  
  temp = DF_gno_times[i,1:3]
...
AllResults_g <- outputDir
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

write.table(x=DF_gno_times, file = paste(AllResults_g, "Genotype.times.txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
