...
my $output_g = '';  ## such as "4_format_bismarkCov.CH/2_ToHuman"
...
my $HELP = '
        ...
        Usage:
               perl  4_format_bismarkCov.CH.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  4_format_bismarkCov.CH.pl   -in 3_Methylation/2_ToHuman    -out 4_format_bismarkCov.CH/2_ToHuman    > 4_format_bismarkCov.CH.runLog.txt 2>&1   &
...
$output_g = '4_format_bismarkCov.CH/2_ToHuman';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  4_format_bismarkCov.CH.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {     
    next unless $inputFiles_g[$i] =~ m/\.CH.txt$/;
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    my $input   = "$input_g/$temp.CH.txt";
    my $output1 = "$output_g/$temp.CHG.cov";
    my $output2 = "$output_g/$temp.CHG.bed";
    my $output1A = "$output_g/$temp.CHH.cov";
    my $output2A = "$output_g/$temp.CHH.bed";

    open(INPUT1,  "<",   $input    )     or  die "$!"; 
    open(OUT1,    ">",   $output1  )     or  die "$!";  
    open(OUT2,    ">",   $output2  )     or  die "$!"; 
    open(OUT1A,    ">",   $output1A  )     or  die "$!";  
    open(OUT2A,    ">",   $output2A  )     or  die "$!"; 
    my @lines1 = <INPUT1>; 
    ...
    for(my $k=1; $k<=$#lines1; $k++ ){
            ...
            ($type eq "CHG")  or  ($type eq "CHH")  or  die;
            ...
            if( ($total > 5) and ($chr =~ m/^chr\d+$/) and ($type eq "CHG") ){
                 print OUT1 "$chr\t$start\t$end\t$percentage\t$methylated\t$unmeth\n";
                 print OUT2 "$chr\t$start\t$end\t$newname\t$percentage\t.\n";
            }

            if( ($total > 5) and ($chr =~ m/^chr\d+$/) and ($type eq "CHH") ){
                 print OUT1A "$chr\t$start\t$end\t$percentage\t$methylated\t$unmeth\n";
                 print OUT2A "$chr\t$start\t$end\t$newname\t$percentage\t.\n";
            }
    }    
