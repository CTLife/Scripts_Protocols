...
my $output_g = '';  ## such as "1_apaiq_identification"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------     
        Usage:
               perl 1_apaiq_identification.pl    [-version]    [-help]    [-in inputFile]    [-out outDir]      
        For instance:
               nohup time  perl 1_apaiq_identification.pl   -in BG_RPM/Input_minus   -out 1_apaiq_identification     > 1_apaiq_identification.runLog.txt  2>&1    &
...
$output_g = '1_apaiq_identification';    ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl 1_apaiq_identification.pl  -help' \n";
    exit 0;
}
...
for ( my $j=0; $j<=$#inputFiles_g; $j++ ) { 
        ...
        ## ISR
        system( "apaiq   --input_plus=$bg_minus  --input_minus=$bg_plus   --out_dir=$output_g/  --fa_file=$genome_fa   --name=$name   --DB_file $polyADB   --model $model   --t 32   --RNASeqRCThreshold  0.05     >  $output_g/$name.runLog 2>&1    "); 
      ##system( "apaiq   --input_plus=$bg_plus   --input_minus=$bg_minus  --out_dir=$output_g/  --fa_file=$genome_fa   --name=$name   --DB_file $polyADB   --model $model   --t 32   --RNASeqRCThreshold  0.05     >  $output_g/$name.runLog 2>&1    "); 
