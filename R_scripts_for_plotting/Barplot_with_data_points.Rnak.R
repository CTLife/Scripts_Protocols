...
df1 <- read.table(file="Rank.txt", header = TRUE, sep = "\t", quote = "\"'",   dec = "." )
dim(df1)
df1[1:5,]

df2 = data.frame( celltype=c(df1$celltype, df1$celltype),  rep=c(df1$rep, df1$rep ), value=c(df1$Rny1, df1$Rny3), RNAtype=c( rep("Rny1", nrow(df1)), rep("Rny3", nrow(df1)) ) )
dim(df2)
df2
