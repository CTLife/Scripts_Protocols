...
outDir_g = "RUVg-normalize-rawCounts_m6A"
...
##############################################################################################################################################################################################
DF_A <- read.table("finalMergedPeaks/IP_forward_merged.bed",  header=TRUE,   sep="\t", comment.char = "" )     
dim( DF_A )
...
rowSD1 = apply(myRPKM2,  1,  sd, na.rm=TRUE)
length(rowSD1)
length(rowSD1[rowSD1==0])
length(rowSD1[rowSD1>5])
length(rowSD1[rowSD1<5])
...
numOfZero1 = apply(myRPKM2,  1, numOfZero )
length(numOfZero1)
length(numOfZero1[numOfZero1>70])
length(numOfZero1[numOfZero1<50])


mybool = (     (numOfZero1<60)  &  (rowSD1>10) )
length( mybool )
length( mybool[mybool] )



myRPKM2 = myRPKM2[mybool , ]
min(myRPKM2)
max(myRPKM2)
dim(myRPKM2)
...
write.table(matrix3,    file = paste(outDir_g, "1d.rawCounts.txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
...
NormalizationStep_RUVg <- function(Set,  K=3) {    
  ...
  ## RUVg
  {
    ...
    ## top[nrow(top),]
    ## rownames(top)[1:10]
    empirical  <- rownames(Set)[  which(  ! ( rownames(Set) %in%  rownames(top)[1:(floor(nrow(top) * (50/100) ) )]    ) ) ]   
    length(empirical)
    ## empirical[1:10]
...
myHierarchicalClustering_1_g(  mat_3three  = RUVg_matrix2,   path_temp1=paste(outDir_g, "4.HierarchicalClustering", sep="/"),  width1=15, height1=6,  groups1 = group_all  )
###########################################################################################################################################################################
...
RUVg_matrix2 <- reset_outliers2(RUVg_matrix2)
