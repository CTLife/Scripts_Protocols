...
my $output_g = '';  ## such as "5A_computeMatrix-point"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Usage:
               perl  5A_computeMatrix-point.pl    [-version]    [-help]      [-in inputDir]     [-bed bedDir]     [-out outDir]
        For instance:
               nohup perl  5A_computeMatrix-point.pl    -in 1_bamCoverage   -bed GenomicRegions    -out 5A_computeMatrix-point    > 5A_computeMatrix-point.runLog.txt 2>&1  &
...
$output_g   = '5A_computeMatrix-point';   ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  5A_computeMatrix-point.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#BED_Files_g; $i++ ) { 
    ...
    for ( my $j=0; $j<=$#BigWigfiles_g; $j++ ) {
         ...
         system("computeMatrix    reference-point    --scoreFileName $input_g/$BigWigfiles_g[$j]    --regionsFileName $BEDdir_g/$BED_Files_g[$i]     --smartLabels   --numberOfProcessors max/2    --verbose    -o $output_sub1/1.gzipped-matrix.gz    --outFileNameMatrix $output_sub1/1.matrix.txt   --outFileSortedRegions  $output_sub1/1.Regions.txt    --referencePoint  center   --upstream 2000   --downstream 2000  --binSize 10    --missingDataAsZero     >> $output_sub1/1.runLog.txt   2>&1");                               
