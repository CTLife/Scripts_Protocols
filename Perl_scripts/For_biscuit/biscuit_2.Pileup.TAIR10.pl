...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "1_finalBAM/2_ToHuman"
my $output_g = '';  ## such as "3_biscuit/1_QC/2_ToHuman"
...
my $HELP = '
        ...
        Usage:
               perl  biscuit_2.Pileup.TAIR10.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  biscuit_2.Pileup.TAIR10.pl   -in 1_finalBAM/2_ToHuman    -out 3_biscuit/2_Pileup/2_ToHuman    > biscuit_2.Pileup.TAIR10.runLog.txt 2>&1   &
...
$input_g  = '1_finalBAM/2_ToHuman';          ## This is only an initialization value or suggesting value, not default value.
$output_g = '8_callMethylation';             ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  biscuit_2.Pileup.TAIR10.pl  -help' \n";
    exit 0;
}
...
my $Bismark_index_g="$commonPath_g/bwa-meth/RefGenomes/TAIR10/Arabidopsis_thaliana.TAIR10.genome.fasta";
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    system(" biscuit pileup  -@ $numCores_g    -b 1  -m 1  -a 1  -c  -u -p    -o  $output_g/$temp.vcf  -w $output_g/$temp.stat   $Bismark_index_g     $input_g/$temp.bam      ");
    system("sleep 2s");
    system("bgzip -@ $numCores_g   $output_g/$temp.vcf ");
    system("sleep 2s");
    system("tabix -p vcf  $output_g/$temp.vcf.gz ");
