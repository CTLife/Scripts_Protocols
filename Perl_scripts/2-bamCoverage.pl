...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "1-BAM/minus_strand"
my $output_g = '';  ## such as "2-bamCoverage/minus_strand"
...
my $HELP = '
        ...
        Usage:
               perl  2-bamCoverage.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl  2-bamCoverage.pl    -in 1-BAM/minus_strand   -out 2-bamCoverage/minus_strand    > 2-bamCoverage.runLog  2>&1   &
...
## Version Infromation
my $version = " version 0.1,  2021-10-30.";
...
$input_g  = '1-BAM/minus_strand';              ## This is only an initialization value or suggesting value, not default value.
$output_g = '2-bamCoverage/minus_strand';      ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  2-bamCoverage.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/.bam$//  or  die;
        system(" bamCoverage   --bam $input_g/$temp.bam     --outFileName $output_g/$temp.bw     --outFileFormat bigwig    --binSize 10    -p 32     --verbose   --normalizeUsing RPKM         > $output_g/$temp.runLog.txt  2>&1  ");                                                  
}
