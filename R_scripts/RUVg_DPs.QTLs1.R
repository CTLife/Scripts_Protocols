...
getParameters_f <- function() {
  option_list_Local <- list(   ## Options list with associated default value.  
    optparse::make_option(opt_str=c("-a", "--groupA"),
                          default="1_BA9/merged.bed",
                          type="character",   dest="groupA",
                          help="target file for group A.  [default: %default]."),
    
    optparse::make_option(opt_str=c("-b", "--groupB"),
                          default="2_BA24/merged.bed",
                          type="character",   dest="groupB",
                          help="target file for group B.  [default: %default]."),
  ...
  parser_Local <- optparse::OptionParser(usage="usage: %prog [options]",
                                         ...
                                         description="Version 0.1, January 10, 2023.",                             
...
## groupA_g      <- "1_BA9/merged.bed"
## groupB_g      <- "2_BA24/merged.bed"
## outDir_g      <- "1_vs_2"
##############################################################################################################################################################################################
...
##############################################################################################################################################################################################
DF_A <- read.table(groupA_g, header=TRUE,   sep="\t", comment.char = "" )  
dim( DF_A )
## DF_A[1:3,1:10]
DF_A1 <- DF_A[, -c(1:6)]
rownames(DF_A1) <- as.vector(DF_A[,4])
colnames(DF_A1) <- as.vector( paste("groupA", colnames(DF_A1), sep=".") )
dim( DF_A1 )
## DF_A1[1:3,1:10]

DF_B <- read.table(groupB_g, header=TRUE,   sep="\t", comment.char = "" )  
dim( DF_B )
## DF_B[1:3,1:10]
DF_B1 <- DF_B[, -c(1:6)]
rownames(DF_B1) <- as.vector(DF_B[,4])
...
myHeader = c( colnames(DF_A1) , colnames(DF_B1) )
myRPKM   = cbind(DF_A1, DF_B1)  ##log2(cpm+1)
myGroups = c( rep("A", ncol(DF_A1)) ,   rep("B", ncol(DF_B1))  ) 
myHeader = rbind( myGroups,  myHeader)
dim( DF_A1 )
dim( DF_B1 )
dim( myHeader  )
dim( myRPKM )
...
write.table(norm_matrix2,    file = paste(AllResults_g, "1c.loggedRPKM.txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
