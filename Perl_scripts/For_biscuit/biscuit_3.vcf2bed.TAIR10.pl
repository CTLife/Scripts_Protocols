...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "2_Pileup/2_ToHuman"
my $output_g = '';  ## such as "3_Methylation/2_ToHuman"
...
my $HELP = '
        ...
        Usage:
               perl  biscuit_3.vcf2bed.hg38.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  biscuit_3.vcf2bed.hg38.pl   -in 2_Pileup/2_ToHuman    -out 3_Methylation/2_ToHuman    > biscuit_3.vcf2bed.hg38.runLog.txt 2>&1   &
...
$input_g  = '2_Pileup/2_ToHuman';          ## This is only an initialization value or suggesting value, not default value.
$output_g = '3_Methylation/2_ToHuman';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  biscuit_3.vcf2bed.hg38.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    system(" nohup  biscuit vcf2bed   -t cg   -k 5  -c  -e  $input_g/$temp.vcf.gz   2> $output_g/$temp.log1.txt  > $output_g/$temp.CpG.txt  &");
    system(" nohup  biscuit vcf2bed   -t ch   -k 5  -c  -e  $input_g/$temp.vcf.gz   2> $output_g/$temp.log2.txt  > $output_g/$temp.CH.txt   &");  
