...
my $output_g = '';  ## such as "5B_computeMatrix-scaleRegions"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Usage:
               perl  5B_computeMatrix-scaleRegions.pl    [-version]    [-help]      [-in inputDir]     [-bed bedDir]     [-out outDir]
        For instance:
               nohup perl  5B_computeMatrix-scaleRegions.pl    -in 1_bamCoverage   -bed GenomicRegions    -out 5B_computeMatrix-scaleRegions    > 5B_computeMatrix-scaleRegions.runLog.txt 2>&1   &
...
$output_g   = '5B_computeMatrix-scaleRegions';   ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  5B_computeMatrix-scaleRegions.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#BED_Files_g; $i++ ) { 
    ...
    for ( my $j=0; $j<=$#BigWigfiles_g; $j++ ) {
         ...
         system("computeMatrix    scale-regions    --scoreFileName $input_g/$BigWigfiles_g[$j]    --regionsFileName $BEDdir_g/$BED_Files_g[$i]     --smartLabels   --numberOfProcessors 16   --verbose    -o $output_sub1/1.gzipped-matrix.gz    --outFileNameMatrix $output_sub1/1.matrix.txt   --outFileSortedRegions  $output_sub1/1.Regions.txt    --regionBodyLength  1000   --upstream 1000   --downstream 1000  --binSize 10    --missingDataAsZero     >> $output_sub1/1.runLog.txt   2>&1");                               
