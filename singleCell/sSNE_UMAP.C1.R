...
length(numOfZero1)
length(numOfZero1[numOfZero1 < 0.8*ncol(DF_value) ])
 

## Number of zeros for each column (gene)
numOfZero2 = apply(DF_value,  2, numOfZero )
length(numOfZero2)
length(numOfZero2[numOfZero2 < 0.9*nrow(DF_value) ])
...
bool1 <- as.vector(  numOfZero1 < 0.8*ncol(DF_value)  )
DF_value2 <- DF_value[ bool1, ] 
dim(DF_value2)

bool2 <- as.vector(  numOfZero2 < 0.9*nrow(DF_value)  )
