...
my $HELP = '
        ...
        Usage:
               perl  biscuit_1.qc.Lambda.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  biscuit_1.qc.Lambda.pl   -in 1_finalBAM/1_ToLambda    -out 3_biscuit/1_QC/1_ToLambda    > biscuit_1.qc.Lambda.runLog.txt 2>&1   &
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  biscuit_1.qc.Lambda.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    system("   biscuit qc   $Lambda_index_g   $input_g/$temp.bam   $output_g/$temp  >  $output2_g/$temp.runLog.txt    2>&1    ");      
    ##system("sleep 3s");                                                                    
}
###################################################################################################################################################################################################
