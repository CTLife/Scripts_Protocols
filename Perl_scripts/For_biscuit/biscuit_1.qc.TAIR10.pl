...
my $HELP = '
        ...
        Usage:
               perl  biscuit_1.qc.TAIR10.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  biscuit_1.qc.TAIR10.pl   -in 1_finalBAM/2_ToHuman    -out 3_biscuit/1_QC/2_ToHuman     > biscuit_1.qc.TAIR10.runLog.txt 2>&1   &
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  biscuit_1.qc.hg38.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    system("   biscuit qc   $Bismark_index_g   $input_g/$temp.bam   $output_g/$temp  >  $output2_g/$temp.runLog.txt    2>&1    ");  
    ##system("bash  QC.sh     --outdir $output_g     hg38_biscuit_qc_assets    $Bismark_index_g     QC.$temp    $input_g/$temp.bam   >  $output2_g/QC.$temp.runLog.txt    2>&1   ");
