...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "2_Pileup/1_ToLambda"
my $output_g = '';  ## such as "3_Methylation/1_ToLambda"
...
my $HELP = '
        ...
        Usage:
               perl  biscuit_3.vcf2bed.Lambda.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  biscuit_3.vcf2bed.Lambda.pl   -in 2_Pileup/1_ToLambda    -out 3_Methylation/1_ToLambda    > biscuit_3.vcf2bed.Lambda.runLog.txt 2>&1   &
...
$input_g  = '2_Pileup/1_ToLambda';          ## This is only an initialization value or suggesting value, not default value.
$output_g = '3_Methylation/1_ToLambda';     ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  biscuit_3.vcf2bed.Lambda.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {     
    next unless $inputFiles_g[$i] =~ m/\.vcf.gz$/;
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    system(" biscuit vcf2bed   -t cg   -k 5  -c  -e  $input_g/$temp.vcf.gz  > $output_g/$temp.CpG.txt ");
    system(" biscuit vcf2bed   -t ch   -k 5  -c  -e  $input_g/$temp.vcf.gz  > $output_g/$temp.CH.txt ");  
    system(" biscuit vcf2bed   -t snp  -k 5  -c  -e  $input_g/$temp.vcf.gz  > $output_g/$temp.SNP.txt ");                                                               
}
###################################################################################################################################################################################################
