...
###################################################################################################################################################################################################
my $input_g  = '';  ## such as "1_finalBAM/1_ToLambda"
my $output_g = '';  ## such as "3_biscuit/1_QC/1_ToLambda"
...
my $HELP = '
        ...
        Usage:
               perl  biscuit_2.Pileup.Lambda.pl    [-version]    [-help]     [-in inputDir]    [-out outDir]
        For instance:
               nohup perl  biscuit_2.Pileup.Lambda.pl   -in 1_finalBAM/1_ToLambda    -out 3_biscuit/2_Pileup/1_ToLambda    > biscuit_2.Pileup.Lambda.runLog.txt 2>&1   &
...
$input_g  = '1_finalBAM/1_ToLambda';          ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  biscuit_2.Pileup.Lambda.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    ...
    system("biscuit pileup  -@ $numCores_g    -b 1  -m 1  -a 1  -c  -u -p    -o  $output_g/$temp.vcf  -w $output_g/$temp.stat   $Lambda_index_g     $input_g/$temp.bam ");
    system("sleep 2s");
