...
my $output_g = '';  ## such as "3B_multiBigwigSummary-BED"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Usage:
               perl  3B_multiBigwigSummary-BED.pl    [-version]    [-help]      [-in inputDir]     [-bed bedDir]     [-out outDir]
        For instance:
               nohup  perl  3B_multiBigwigSummary-BED.pl    -in 1_bamCoverage   -bed GenomicRegions    -out 3B_multiBigwigSummary-BED    > 3B_multiBigwigSummary-BED.runLog.txt 2>&1   &
        ...
        For more details please visit https://github.com/CTLife/Scripts_Protocols/tree/main/deepTools
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
';
...
$output_g   = '3B_multiBigwigSummary-BED';   ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  3B_multiBigwigSummary-BED.pl  -help' \n";
    exit 0;
}
...
###################################################################################################################################################################################################
say   "\n\n\n\n\n\n##################################################################################################";
say   "Using multiBigwigSummary BED-file, plotCorrelation and plotPCA ......";
...
for ( my $i=0; $i<=$#BED_Files_g; $i++ ) { 
    ...
    system("multiBigwigSummary  BED-file    --bwfiles @BigWigfiles_g2    --BED $BEDdir_g/$BED_Files_g[$i]  --smartLabels   --numberOfProcessors 16    --verbose    --outRawCounts $output_sub1/matrix.txt   --outFileName $output_sub1/1-results.npz    >> $output_sub1/1-runLog.txt   2>&1");                               
    ...
    system("plotPCA         -in $output_sub1/1-results.npz     --ntop 100                                        -o $output_sub1/6B-top5000-plotPCA.pdf                  --outFileNameData  $output_sub1/6B-top10000-plotPCA.txt                   --plotHeight 20   --plotWidth 20      >> $output_sub1/6B-plotPCA-runLog.txt   2>&1");                               
    ...
    system("plotPCA         -in $output_sub1/1-results.npz     --ntop 100     --rowCenter                        -o $output_sub1/7B-top5000-plotPCA.rowCenter.pdf        --outFileNameData  $output_sub1/7B-top10000-plotPCA.rowCenter.txt         --plotHeight 20   --plotWidth 20      >> $output_sub1/7B-plotPCA-runLog.txt   2>&1");                               
